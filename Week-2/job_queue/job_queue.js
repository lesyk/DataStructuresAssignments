var Maker = (function () {
    function Maker(id) {
        this.priority = 0;
        this.id = id;
    }
    return Maker;
}());
var Heap = (function () {
    function Heap() {
        this.data = [];
        this.swaps = [];
        this.swapsCount = 0;
        this.size = 0;
    }
    Heap.prototype.insert = function (id) {
        this.data[this.size] = new Maker(id);
        this.siftUp(this.size);
        this.size++;
    };
    Heap.prototype.parent = function (i) {
        if (i === 0) {
            return 0;
        }
        else {
            return parseInt(i / 2);
        }
    };
    Heap.prototype.leftChild = function (i) {
        return (2 * i) + 1;
    };
    Heap.prototype.rightChild = function (i) {
        return (2 * i) + 2;
    };
    Heap.prototype.siftUp = function (i) {
        while (i > 0 && this.data[i].priority < this.data[this.parent(i)].priority) {
            this.swap(i, this.parent(i));
            i = this.parent(i);
        }
    };
    Heap.prototype.getMax = function () {
        return this.data[0];
    };
    Heap.prototype.siftDown = function (i) {
        var maxIndex = i;
        var leftChild = this.leftChild(i);
        var rightChild = this.rightChild(i);
        if (leftChild < this.size && this.data[leftChild].priority < this.data[maxIndex].priority) {
            maxIndex = leftChild;
        }
        if (rightChild < this.size && this.data[rightChild].priority < this.data[maxIndex].priority) {
            maxIndex = rightChild;
        }
        if (i !== maxIndex) {
            this.swap(i, maxIndex);
            this.siftDown(maxIndex);
        }
    };
    Heap.prototype.swap = function (i1, i2) {
        var tmp = this.data[i1];
        this.swaps.push((i1) + ' ' + (i2));
        this.swapsCount += 1;
        this.data[i1] = this.data[i2];
        this.data[i2] = tmp;
    };
    Heap.prototype.exctractMax = function () {
        var res = this.data[0];
        this.data[0] = this.data[this.size - 1];
        this.data.pop();
        this.size--;
        this.siftDown(0);
        return res;
    };
    Heap.prototype.changePriority = function (i, p) {
        var oldP = this.data[i].priority;
        this.data[i].priority = p;
        if (p > oldP) {
            this.siftDown(i);
        }
        else {
            this.siftUp(i);
        }
    };
    return Heap;
}());
// let h = new Heap()
// h.insert(1)
// h.insert(2)
// h.insert(3)
// h.insert(4)
// h.insert(5)
// console.log(h.data)
// console.log(h.getMax())
// console.log(h.exctractMax())
// console.log(h.data)
// console.log(h.getMax())
var JobQueue = (function () {
    function JobQueue(line) {
        this.numberOfWorkers = 0;
        this.globalTimeSpent = 0;
        this.assignWorkers = [];
        this.startTimes = [];
        this.next_free_time = [];
        this.jobs = [];
        this.workers = new Heap();
        var t = line.split(' ').map(function (el) { return parseInt(el, 10); });
        this.numberOfWorkers = t[0];
    }
    JobQueue.prototype.setJobs = function (line) {
        this.jobs = line.split(' ').map(function (el) { return (parseInt(el, 10)); });
    };
    JobQueue.prototype.assignJobs = function () {
        for (var i = 0; i < this.numberOfWorkers; i++) {
            this.workers.insert(i);
        }
        for (var i = 0; i < this.jobs.length; i++) {
            var duration = this.jobs[i];
            var worker = this.workers.getMax();
            this.assignWorkers[i] = worker.id;
            if (this.next_free_time[worker.id] === undefined) {
                this.next_free_time[worker.id] = 0;
            }
            this.startTimes[i] = this.next_free_time[worker.id];
            var time = worker.priority + duration;
            this.next_free_time[worker.id] = +time;
            this.workers.changePriority(0, time);
        }
    };
    JobQueue.prototype.printQ = function () {
        for (var i = 0; i < this.jobs.length; i++) {
            console.log(this.assignWorkers[i] + " " + this.startTimes[i]);
        }
    };
    return JobQueue;
}());
var readline = require('readline');
var rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
    terminal: false
});
var lineNumber = 1;
var q;
rl.on('line', function (line) {
    if (lineNumber === 1) {
        q = new JobQueue(line);
    }
    if (lineNumber === 2) {
        // console.time('100-elements');
        q.setJobs(line);
        q.assignJobs();
        q.printQ();
        // console.timeEnd('100-elements');
        process.exit();
    }
    lineNumber++;
});
